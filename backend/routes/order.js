...
async function syncToProfessionalTables(transaction, tableId, displayOrderId, items, userId) {
  ...
  let bizId = "00000000-0000-0000-0000-000000000000";
...
// ✅ ASSIGN TAKEAWAY TABLE FOR QR SCAN
router.post("/assign-takeaway-table", async (req, res) => {

  console.log("🔥 ASSIGN TAKEAWAY API HIT");
  let transaction;
  try {
    const pool = await poolPromise;
    ...
    // Find the first available takeaway table using UPDLOCK to avoid race conditions
    const tableResult = await transaction.request().query(`
      SELECT TOP 1 TableId, TableNumber 
      FROM TableMaster WITH (UPDLOCK) 
      WHERE IstakeAway = 1 AND DiningSection = 4 AND CurrentOrderId IS NULL 
      ORDER BY SortCode ASC
    `);

    if (tableResult.recordset.length === 0) {
      await transaction.rollback();
      return res.status(404).json({ success: false, message: "All takeaway tables are currently occupied." });
    }

    const table = tableResult.recordset[0];
    const cleanTableId = String(table.TableId).replace(/^\\{|\\}$/g, "").trim();
    ...
    const seqResult = await transaction.request()
      .input("RestId", sql.UniqueIdentifier, String(bizId))
      .input("Today", sql.Date, todayStr)
      .query(`
        IF NOT EXISTS (SELECT 1 FROM OrderSequences WHERE RestaurantId = @RestId AND SequenceDate = @Today)
        BEGIN
        ...
        WHERE RestaurantId = @RestId AND SequenceDate = @Today;
      `);
    const dailySequence = seqResult.recordset[0]?.LastNumber || 1;
    const displayOrderId = `${datePrefix}-${String(dailySequence).padStart(4, '0')}`;
    ...
    res.json({ success: true, tableId: cleanTableId, tableNumber: table.TableNumber, orderId: displayOrderId });
  } catch (err) {
    if (transaction) {
      try {
        await transaction.rollback();
    ...
    console.error("Assign Takeaway Table Error:", err);
    res.status(500).json({ success: false, message: "Server error during table assignment" });
  }
});
