...
function App() {
  ...
  const [svcIdentification, setSvcIdentification] = useState(0);
  ...
  // OPEN modifier inline-text states
  const [showOpenInput, setShowOpenInput] = useState(false);
  const [openModText, setOpenModText] = useState("");
    ...
    const loadAppSettings = async () => {
      try {
        ...
        if (data.success) {
          ...
          setSvcIdentification(Number(data.svcIdentification) === 1 ? 1 : 0);
  ...
  const calculateComboTotal = () => {
    ...
    const modifierPriceTotal = chosenModifiers.reduce((sum, m) => {
      if (Number(m.isPriceAffect) === 1 && Number(m.isDishPrice) === 1 && Number(m.DishCost || 0) > 0) {
        return sum + Number(m.DishCost);
      }
      return sum;
    }, 0);
  ...
  const decreaseQty = async (index) => {
    ...
    if (currentQty <= 1) {
      ...
      try {
        ...
        // ✅ BUG FIX: Only reload cart from DB if cart still has items.
        // Calling loadCart after emptying the cart causes a race condition:
        // the async DB fetch returns [] and overwrites newly added items.
        // The optimistic setCart([]) above already handles the empty state.
        setCart(prev => {
          if (prev.length > 0 && tableId) {
            loadCart(tableId);
          }
          return prev;
        });
  ...
  const handlePayOnline = async () => {
    ...
    const eligibleAmount = cart.reduce(
      (sum, item) =>
        (Number(item.IsServiceCharge) === 1 || item.IsServiceCharge === true || Number(item.isServiceCharge) === 1 || item.isServiceCharge === true)
          ? sum +
  ...
  const loadCart = async (tableId) => {
    ...
    try {
      ...
      if (data.items) {
        ...
        // ✅ BUG FIX: Do NOT overwrite a non-empty local cart with an empty DB response.
        // This prevents a race condition where a newly added item (optimistic UI update)
        // gets wiped by a stale loadCart response returning [] from the DB.
  ...
  const toggleModifier = (mod) => {
    if (mod.ModifierName.toUpperCase() === "OPEN") {
      if (showOpenInput) {
        // Clicking OPEN again — close and deselect
        setShowOpenInput(false);
        setOpenModText("");
        setSelectedModifierIds((prev) => prev.filter((id) => id !== "open"));
      } else {
        // Open the inline text input and mark OPEN as selected
        setShowOpenInput(true);
        setSelectedModifierIds((prev) =>
          prev.includes("open") ? prev : [...prev, "open"]
        );
      }
      return;
    }
  ...
  const addWithModifiers = () => {
    ...
    let selectedMods = allAvailable.filter((m) =>
      selectedModifierIds.includes(m.ModifierID)
    );

    // Embed the typed text into the OPEN modifier name
    if (showOpenInput && openModText.trim()) {
      selectedMods = selectedMods.map((m) =>
        m.ModifierID === "open"
          ? { ...m, ModifierName: `OPEN: ${openModText.trim()}` }
          : m
      );
    } else {
      // If OPEN was selected but no text typed, remove it from the list
      selectedMods = selectedMods.filter((m) => m.ModifierID !== "open");
    }
        ...
        element={
          ...
          <SettingsIcon />
                  ...
                  <div className="dish-list">
                    {filteredItems.map((dish) => (
                      <div className="dish-card" key={dish.DishId} onClick={() => openModifiers(dish)}
                        style={{
                          backgroundColor: (Number(dish.IsServiceCharge) === 1 || dish.IsServiceCharge === true || Number(dish.isServiceCharge) === 1 || dish.isServiceCharge === true) ? "#FFF3F3" : "#fff",

                          border: (Number(dish.IsServiceCharge) === 1 || dish.IsServiceCharge === true || Number(dish.isServiceCharge) === 1 || dish.isServiceCharge === true)
                            ? "2px solid #FF6B6B"
                            : "1px solid #E5E7EB",
                        }}
                      >
                        ...
                        <div
                          className="dish-name"
                          style={{
                            color: (Number(dish.IsServiceCharge) === 1 || dish.IsServiceCharge === true || Number(dish.isServiceCharge) === 1 || dish.isServiceCharge === true)
                              ? "#D32F2F"
                              : "#111827",
                          }}
                        >
                        ...
                        <div
                          ...
                          style={{
                            color: (Number(dish.IsServiceCharge) === 1 || dish.IsServiceCharge === true || Number(dish.isServiceCharge) === 1 || dish.isServiceCharge === true) ? "#D32F2F" : "#F97316",
                          }}
                        >
                        ...
                        <div className="cart-items-list">
                          {cart.map((item, index) => (
                            <div
                              ...
                              className="cart-item"
                              style={{
                                background: (Number(item.IsServiceCharge) === 1 || item.IsServiceCharge === true || Number(item.isServiceCharge) === 1 || item.isServiceCharge === true) ? "#FFF3F3" : "#fff",
                                border: (Number(item.IsServiceCharge) === 1 || item.IsServiceCharge === true || Number(item.isServiceCharge) === 1 || item.isServiceCharge === true)
                                  ? "2px solid #FF6B6B"
                                  : "1px solid #E5E7EB",
                                borderRadius: "16px",
                                marginBottom: "12px",
                            ...
                            >
                              ...
                              <div className="ci-info">
                                ...
                                <div className="ci-name">
                                  ...
                                  <div className="ci-title" style={{ color: (Number(item.IsServiceCharge) === 1 || item.IsServiceCharge === true || Number(item.isServiceCharge) === 1 || item.isServiceCharge === true) ? "#D32F2F" : "#111827" }}>
                                    {item.Name || item.name}
                                  </div>
                                  ...
                                  {item.selectedMods?.length > 0 && (
                                    <div className="ci-mods">
                                      {item.selectedMods.map((m, index) => (
                                        <div key={`${m.ModifierID}-${index}`} style={{ marginLeft: "0", color: "#666", fontSize: "13px" }}>
                                          &bull; {m.ModifierName}
                              ...
                              <div className="ci-price" style={{ color: (Number(item.IsServiceCharge) === 1 || item.IsServiceCharge === true || Number(item.isServiceCharge) === 1 || item.isServiceCharge === true) ? "#D32F2F" : "#f97316" }}>
                                $
                                {(
                                  Number(item.Price || item.price || 0) *
                                  Number(item.qty || 1)
                            ...
                            <div className="pp-promo-card">
                              ...
                              <div className="pp-promo-details">
                                ...
                                <div className="pp-promo-row pp-promo-payable-row">
                                  ...
                                  <span className="pp-promo-payable">${(subTotal + serviceCharge + gstAmount - availableCredit).toFixed(2)}</span>
                                </div>
                              </div>
                            </div>
                          )}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              {applyPromo && availableCredit > 0 && (
                                <span className="pp-total-original" style={{ fontSize: '14px' }}>
                                  ${(subTotal + serviceCharge + gstAmount).toFixed(2)}
                                </span>
                              )}
              ...
              {showModifier && selectedDish && (
                <div className="modal-overlay" onClick={() => { setShowModifier(false); setShowOpenInput(false); setOpenModText(""); }}>
                  <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                    <div className="modal-header">
                      <h2 className="modal-title">Modifiers for {selectedDish.Name || selectedDish.name}</h2>
                      <button className="modal-close" onClick={() => { setShowModifier(false); setShowOpenInput(false); setOpenModText(""); }}>
                        &times;
                      </button>
                    </div>
                          ...
                          <div key={m.ModifierID}>
                            ...
                            >
                              ...
                              <div
                                className={`checkbox ${
                                  m.ModifierName.toUpperCase() === "OPEN"
                                    ? showOpenInput ? "active" : ""
                                    : selectedModifierIds.includes(m.ModifierID) ? "active" : ""
                                }`}
                              >
                                {(m.ModifierName.toUpperCase() === "OPEN" ? showOpenInput : selectedModifierIds.includes(m.ModifierID)) && (
                                  <span className="checkmark">&#10003;</span>
                                )}
                              </div>
                            </div>
                            {/* Inline OPEN text input */}
                            {m.ModifierName.toUpperCase() === "OPEN" && showOpenInput && (
                              <div
                                style={{
                              ...
                              >
                                <input
                                  type="text"
                                  autoFocus
                                  placeholder="Enter open instruction…"
                                  value={openModText}
                                  onChange={(e) => setOpenModText(e.target.value)}
                                  style={{
                                    width: '100%',
                                    padding: '9px 12px',
                                    borderRadius: '8px',
